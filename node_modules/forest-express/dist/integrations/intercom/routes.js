...
module.exports = function (app, model, Implementation, opts) {
  ...
  if (opts.integrations && opts.integrations.intercom) {
    integrationInfo = new IntegrationInformationsGetter(modelName, Implementation, opts.integrations.intercom).perform();
  }

  this.getAttributes = function (request, response, next) {
    new AttributesGetter(Implementation, _.extend(request.query, request.params), opts, integrationInfo).perform().then(function (attributes) {
  ...
  this.listConversations = function (request, response, next) {
    new ConversationsGetter(Implementation, _.extend(request.query, request.params), opts, integrationInfo).perform().spread(function (count, conversations) {
  ...
  this.getConversation = function (request, response, next) {
    new ConversationGetter(Implementation, _.extend(request.query, request.params), opts).perform().then(function (conversation) {
  ...
  this.perform = function () {
    if (integrationInfo) {
      app.get('/forest/' + modelName + '/:recordId/intercom_attributes', auth.ensureAuthenticated, this.getAttributes);
      app.get('/forest/' + modelName + '/:recordId/intercom_conversations', auth.ensureAuthenticated, this.listConversations);
      app.get('/forest/' + modelName + '_intercom_conversations/:conversationId', auth.ensureAuthenticated, this.getConversation);
    }
  };
};