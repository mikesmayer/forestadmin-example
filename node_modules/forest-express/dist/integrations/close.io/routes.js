...
module.exports = function (app, model, Implementation, opts) {
  ...
  this.perform = function () {
    if (integrationInfo) {
      app.get(path.generate(modelName + '_closeio_leads/:leadId', opts), auth.ensureAuthenticated, closeioLead);

      app.get(path.generate(modelName + '_closeio_leads/:leadId/emails', opts), auth.ensureAuthenticated, closeioLeadEmails);

      app.get(path.generate(modelName + '_closeio_emails/:emailId', opts), auth.ensureAuthenticated, closeioLeadEmail);

      app.get(path.generate(modelName + '/:recordId/lead', opts), auth.ensureAuthenticated, customerLead);

      app.post(path.generate(modelName + '_closeio_leads', opts), auth.ensureAuthenticated, createCloseioLead);
    }
  };
};