...
function Checker(opts, Implementation) {
  ...
  function isProperlyIntegrated() {
    return opts.integrations.stripe.apiKey && opts.integrations.stripe.stripe && opts.integrations.stripe.mapping;
  }

  function isIntegrationDeprecated() {
    var integrationValid = opts.integrations.stripe.apiKey && opts.integrations.stripe.stripe && (opts.integrations.stripe.userCollection || opts.integrations.stripe.userCollection);
  ...
  function isMappingValid() {
    ...
    if (!mappingValid) {
      logger.error('Cannot find some Stripe integration mapping values (' + opts.integrations.stripe.mapping + ') among the project models:\n' + _.keys(models).join(', '));
    }
      ...
      dest[field.field] = {
        ...
        relationshipLinks: {
          related: function related(dataSet) {
            return {
              href: '/forest/' + Implementation.getModelName(model) + '/' + dataSet[schema.idField] + '/' + field.field
            };
          }
        }
      };
