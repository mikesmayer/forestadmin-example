...
module.exports = function (app, model, Implementation, opts) {
  ...
  this.perform = function () {
    if (integrationInfo) {
      app.get(path.generate(modelName + '_stripe_payments', opts), auth.ensureAuthenticated, this.payments);

      app.get(path.generate(modelName + '_stripe_payments/:paymentId', opts), auth.ensureAuthenticated, this.payment);

      app.get(path.generate(modelName + '/:recordId/stripe_payments', opts), auth.ensureAuthenticated, this.payments);

      app.post(path.generate(modelName + '_stripe_payments/refunds', opts), auth.ensureAuthenticated, this.refund);

      app.get(path.generate(modelName + '_stripe_invoices', opts), auth.ensureAuthenticated, this.invoices);

      app.get(path.generate(modelName + '/:recordId/stripe_invoices', opts), auth.ensureAuthenticated, this.invoices);

      app.get(path.generate(modelName + '_stripe_invoices/:invoiceId', opts), auth.ensureAuthenticated, this.invoice);

      app.get(path.generate(modelName + '/:recordId/stripe_cards', opts), auth.ensureAuthenticated, this.cards);

      app.get(path.generate(modelName + '_stripe_cards', opts), auth.ensureAuthenticated, this.card);

      app.get(path.generate(modelName + '_stripe_subscriptions', opts), auth.ensureAuthenticated, this.subscriptions);

      app.get(path.generate(modelName + '_stripe_subscriptions/:subscriptionId', opts), auth.ensureAuthenticated, this.subscription);

      app.get(path.generate(modelName + '/:recordId/stripe_subscriptions', opts), auth.ensureAuthenticated, this.subscriptions);

      app.get(path.generate(modelName + '/:recordId/stripe_bank_accounts', opts), auth.ensureAuthenticated, this.bankAccounts);

      app.get(path.generate(modelName + '_stripe_bank_accounts', opts), auth.ensureAuthenticated, this.bankAccount);
    }
  };
};