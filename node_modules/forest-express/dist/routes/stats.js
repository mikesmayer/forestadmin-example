...
module.exports = function (app, model, Implementation, opts) {
  ...
  this.get = function (request, response, next) {
    ...
    switch (request.body.type) {
      case 'Value':
        promise = new Implementation.ValueStatGetter(model, request.body, opts).perform();
        break;
      case 'Pie':
        promise = new Implementation.PieStatGetter(model, request.body, opts).perform();
        break;
      case 'Line':
        promise = new Implementation.LineStatGetter(model, request.body, opts).perform();
        break;
    }
  ...
  function getErrorQueryColumnsName(result, keyNames) {
    var message = 'The result columns must be named ' + keyNames + ' instead of \'' + Object.keys(result).join('\', \'') + '\'.';
